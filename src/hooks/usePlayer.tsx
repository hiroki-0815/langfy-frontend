...
type Player = {
  url: string | MediaStream;
  muted: boolean; // This flag represents whether the audio track is disabled (for sending)
  playing: boolean;
};
...
const usePlayer = (
  ...
  const toggleAudio = useCallback(() => {
    ...
    setPlayers((prev) => {
      ...
      // Flip the "muted" flag which controls whether the audio track is enabled.
      const newMuted = !currentLocal.muted;

      stream.getAudioTracks().forEach((track) => {
        track.enabled = !newMuted; // If newMuted is false, then track.enabled = true so audio is sent
      });
